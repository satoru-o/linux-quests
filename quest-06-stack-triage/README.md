...
**ゴールは`curl http://localhost:8080/report`が集計結果を返すこと。** そこまで繋がれば一周できている。

## これは何の練習か

実際の障害対応は「1個直したら終わり」であることの方が少ない。1個直すと次の不具合が顔を出す、いわゆる玉ねぎ状態が普通で、そのたびに**どの層が壊れているのか**を切り分け直す必要がある。

このクエストは意図的に複数の層が同時に壊してある。**FLAGは全部で3つ**、それぞれ別の層を復旧させたときに初めて手に入る。

第1層と第2層は独立していて、**どちらから手を付けてもよい**(片方を直せば、もう片方が壊れたままでもその分の手応えは返ってくる)。第3層はその2つが両方通ってから顔を出す。
...
## 第2層：clientからserverが見えていない

`docker compose logs client`を見ると、clientは`server`という名前をそもそも引けていない(第1層とは独立した別の問題なので、先にこちらから手を付けても構わない)。

<details>
<summary>ヒント2-1</summary>

「名前が引けない」と「名前は引けたが繋がらない」はまったく別の話で、疑う層が変わる。前者はDNS(そもそも相手が見えているか)、後者はTCP(見えているが待ち受けていない/弾かれている)。clientのログがどちらを言っているか確認する。
...
第1層と第2層が両方直ると、clientはデータを取得してレポートを書き出せるようになる(`docker compose logs client`に書き出しのログが出る)。しかしゴールの`curl http://localhost:8080/report`はまだ成功しない。
